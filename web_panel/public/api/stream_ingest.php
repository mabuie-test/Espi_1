...
$event = $_POST['event'] ?? ($raw['event'] ?? 'frame');
$fileName = basename((string)($_POST['session'] ?? $_POST['file'] ?? ($raw['session'] ?? $raw['file'] ?? ('live_' . $userId))));
$mediaType = (string)($_POST['mode'] ?? ($raw['mode'] ?? 'video_audio'));
$status = (string)($_POST['status'] ?? ($raw['status'] ?? 'connected'));
$source = (string)($_POST['source'] ?? ($raw['source'] ?? 'auto'));
...
$row = $select->fetch();
if ($row) {
    $upd = db()->prepare('UPDATE media_sessions SET stream_status = ?, bytes_received = bytes_received + ?, source_name = ?, device_id = COALESCE(?, device_id), media_type = ? WHERE id = ?');
    $upd->execute([$status, $frameBytes, $source, $deviceId, $mediaType, (int)$row['id']]);
} else {
